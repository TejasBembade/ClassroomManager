      ...
      <div className="p-6">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="bg-white p-5 rounded-xl shadow-sm">
              ...
              <ul className="mt-4 text-sm text-gray-600 space-y-1">
                {departments.map((d) => (
                  <li key={d._id} className="bg-gray-50 px-3 py-1 rounded flex justify-between items-center">
                    <span>• {d.name}</span>
                    <button onClick={() => handleDelete(deleteDepartment, d._id, `Delete department "${d.name}"?\n\nThis will also delete all its subjects, class assignments, room allocations, locks and instructor accounts.`)}
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm">
              ...
              <ul className="mt-4 text-sm text-gray-600 space-y-1">
                {rooms.map((r) => (
                  <li key={r._id} className="bg-gray-50 px-3 py-1 rounded flex justify-between items-center">
                    <span>• {r.roomNumber}</span>
                    <button onClick={() => handleDelete(deleteRoom, r._id, `Delete room "${r.roomNumber}"?\n\nAll allocations and class assignments in this room will also be deleted.`)}
                      className="text-red-400 hover:text-red-600 text-xs">Delete</button>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-white p-5 rounded-xl shadow-sm">
              ...
              <ul className="mt-4 text-sm text-gray-600 space-y-1">
                {timeSlots.map((t) => (
                  <li key={t._id} className="bg-gray-50 px-3 py-1 rounded flex justify-between items-center">
                    <span>• {t.startTime} - {t.endTime}</span>
                    <button onClick={() => handleDelete(deleteTimeSlot, t._id, `Delete time slot "${t.startTime} - ${t.endTime}"?\n\nAll allocations and assignments in this slot will also be deleted.`)}
                      className="text-red-400 hover:text-red-600 text-xs">Delete</button>
                  </li>
                ))}
              </ul>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="p-2 text-left">Room</th><th className="p-2 text-left">Department</th>
                    <th className="p-2 text-left">Day</th><th className="p-2 text-left">Time</th>
                ...
                <tbody>
                  {allocations.map((a) => (
                    <tr key={a._id} className="border-t">
                      ...
                      <td className="p-2">
                        <button onClick={() => handleDelete(deleteRoomAllocation, a._id, `Remove this allocation?\n\nAny class assigned to this slot will also be deleted.`)}
                          className="text-red-400 hover:text-red-600 text-xs">Delete</button>
                      </td>
                    </tr>
                  ))}
        ...
        {tab === "instructor" && (
          <div className="grid grid-cols-2 gap-6">
            ...
            {/* Instructor list with delete — NEW */}
            <div className="bg-white p-5 rounded-xl shadow-sm">
              <h2 className="font-semibold text-gray-700 mb-4">All Instructors</h2>
              {instructors.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No instructors yet</p>}
              <div className="space-y-2">
                {instructors.map(i => (
                  <div key={i._id} className="bg-gray-50 px-3 py-2 rounded-lg flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-gray-800">{i.name}</p>
                      <p className="text-xs text-gray-500">{i.email} · {i.departmentId?.name}</p>
                    </div>
                    <button onClick={() => handleDelete(deleteInstructor, i._id, `Delete instructor "${i.name}" (${i.email})?`)}
                      className="text-red-400 hover:text-red-600 text-xs ml-3">Delete</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="max-w-md">
            <div className="bg-white p-5 rounded-xl shadow-sm">
              ...
              <form onSubmit={handleLock} className="flex flex-col gap-3">
                ...
                <select className="border rounded-lg px-3 py-2 text-sm" value={lockForm.isLocked}
        ...
        {tab === "timetable" && (
          <div className="bg-white p-5 rounded-xl shadow-sm overflow-auto">
            ...
            {["Monday","Tuesday","Wednesday","Thursday","Friday"].map(day => {
              ...
              return (
                <div key={day} className="mb-8">
                  <h3 className="font-medium text-indigo-600 mb-2 text-base">{day}</h3>
                    ...
                    <tbody>
                      {dayAssignments.map(a => (
                        <tr key={a._id} className="border-t hover:bg-gray-50">
                          <td className="p-2 border font-medium">{a.roomId?.roomNumber || <span className="text-red-400 text-xs">Room deleted</span>}</td>
                          <td className="p-2 border">{a.subjectId?.name || <span className="text-red-400 text-xs">Subject deleted</span>}</td>
                          <td className="p-2 border">{a.teacherName}</td>
                          <td className="p-2 border">{a.timeSlotId?.startTime} - {a.timeSlotId?.endTime}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {assignments.length === 0 && <p className="text-center text-gray-400 py-8">No assignments yet</p>}
          </div>
        )}
      </div>
      <Footer />
